...
class tmhOAuth {
    ...
    private function prepare_oauth_signature() {
        ...
        if(function_exists('base_64_encode')) {
           $encode = base_64_encode(hash_hmac(
                'sha1', $this->request_settings['basestring'], $this->request_settings['signing_key'], true
            ));
        }
        $this->request_settings['oauth1_params']['oauth_signature'] = $this->safe_encode($encode);
    }
    ...
    public function bearer_token_credentials() {
        ...
        if(function_exists('base_64_encode')) {
            return base_64_encode($credentials);
        }
        return;
