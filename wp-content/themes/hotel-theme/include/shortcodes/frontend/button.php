...
if (!function_exists('cs_button_shortcode')) {
    function cs_button_shortcode($atts) {
        ...
        if($button_size =='btn-xlg'){
            ...
            $button_size = 'custom-btn btn-sm';
        }else{
            $button_size = 'custom-btn btn-sml';
        }
        ...
        $html .= '<a href="' . $button_link. '" class="default '.sanitize_html_class($button_type_class). ' '.($button_class_position). ' ' .$button_size. ' bg-color ' . $cs_button_class. '  '.$has_icon.'" style="'.$border.'  background-color: ' . $button_bg_color . '; color:' . $button_color . ';" target="'.$button_target.'">';
        if(isset($button_icon) && $button_icon <> ''){
    ...
    if(function_exists('cs_shortcode_add')){
        cs_shortcode_add(CS_SC_BUTTON, 'cs_button_shortcode');
