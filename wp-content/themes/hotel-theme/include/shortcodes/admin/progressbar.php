...
if ( ! function_exists( 'cs_pb_progressbars' ) ) {
    function cs_pb_progressbars($die = 0){
        ...
        $defaults = array('column_size'=>'1/1','cs_progressbars_style'=>'skills-sec','progressbars_class'=>'');
...
<div id="<?php echo esc_attr($name.$cs_counter);?>_del" class="column  parentdelete <?php echo esc_attr($coloumn_class);?> <?php echo esc_attr($shortcode_view);?>" item="gallery" data="<?php echo cs_element_size_data_array_index($progressbars_element_size)?>" >
  ...
  <div class="cs-wrapp-class-<?php echo esc_attr($cs_counter)?> <?php echo esc_attr($shortcode_element);?>" id="<?php echo esc_attr($name.$cs_counter);?>" style="display: none;">
    <div class="cs-heading-area">
      <h5><?php _e('Edit Progress bars Options','luxury-hotel');?></h5>
        ...
        <div id="shortcode-item-<?php echo esc_attr($cs_counter);?>" data-shortcode-template="{{child_shortcode}} [/<?php echo esc_attr( CS_SC_PROGRESSBAR ) ;?>]" data-shortcode-child-template="[<?php echo esc_attr( CS_SC_PROGRESSBARITEM ) ;?> {{attributes}}] {{content}} [/<?php echo esc_attr( CS_SC_PROGRESSBARITEM ) ;?>]">
          <div class="cs-wrapp-clone cs-shortcode-wrapp cs-disable-true cs-pbwp-content" data-template="[<?php echo esc_attr( CS_SC_PROGRESSBAR ) ;?> {{attributes}}]">
            ...
            <ul class="form-elements" style="display:none">
              <li class="to-label">
                <label><?php _e('Progress Bars Style','luxury-hotel');?></label>
              </li>
              <li class="to-field select-style">
                <select class="cs_progressbars_style" name="cs_progressbars_style[]">
                 <option value="plain-progressbar" <?php if($cs_progressbars_style=='plain-progressbar'){echo 'selected="selected"';}?>><?php _e('Plain Progress bar','luxury-hotel');?></option>
                  <option value="round-strip-progressbar" <?php if($cs_progressbars_style=='round-strip-progressbar'){echo 'selected="selected"';}?>><?php _e('Strip Progress bar','luxury-hotel');?></option>
                  <option value="strip-progressbar" <?php if($cs_progressbars_style=='strip-progressbar'){echo 'selected="selected"';}?>><?php _e('Pattern Progress bar','luxury-hotel');?></option>
       ...
       <?php
        if ( isset($progressbars_num) && $progressbars_num <> '' && isset($atts_content) && is_array($atts_content)){
          ...
          echo '<div class="cs-wrapp-clone cs-shortcode-wrapp cs-pbwp-content" id="cs_infobox_'.$rand_id.'">'; ?>
            <header>
              <h4><i class='icon-arrows'></i><?php _e('ProgressBar','luxury-hotel');?></h4>
              <a href='#' class='deleteit_node'><i class='icon-minus-circle'></i><?php _e('Remove','luxury-hotel');?></a></header>
            <ul class="form-elements">
              <li class="to-label">
                <label><?php _e('ProgressBars Title','luxury-hotel');?></label>
              </li>
              <li class="to-field">
            ...
            <ul class="form-elements">
              <li class="to-label">
                <label><?php _e('Skill (in percentage)','luxury-hotel');?></label>
              </li>
              <li class="to-field">
                ...
                <div class='left-info'><p><?php _e('Set the Skill (In %)','luxury-hotel');?></p></div>
              </li>
            </ul>
            <ul class="form-elements">
              <li class="to-label">
                <label><?php _e('Progress Bars Color','luxury-hotel');?></label>
              </li>

              <li class="to-field">
                <input type="text" name="progressbars_color[]" class="bg_color" value="<?php echo balanceTags(cs_data_validation($progressbars_color)); ?>" />
              </li>
            </ul>
          </div>
          <?php
        ...
        <div class="wrapptabbox" style="padding:0">
          <div class="opt-conts">
            <ul class="form-elements noborder">
              <li class="to-field"> <a href="#" class="add_servicesss cs-main-btn" onclick="cs_shortcode_element_ajax_call('progressbars', 'shortcode-item-<?php echo esc_js($cs_counter);?>', '<?php echo esc_js(admin_url('admin-ajax.php'));?>')"><i class="icon-plus-circle"></i><?php _e('Add Progress bar','luxury-hotel');?></a> </li>
