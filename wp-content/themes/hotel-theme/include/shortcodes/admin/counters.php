...
<div id="<?php echo esc_attr($name.$cs_counter);?>_del" class="column  parentdelete <?php echo cs_allow_special_char($coloumn_class);?> <?php echo cs_allow_special_char($shortcode_view);?>" item="counter" data="<?php echo cs_element_size_data_array_index($counter_element_size)?>" >
  ...
  <div class="cs-wrapp-class-<?php echo esc_attr($cs_counter);?> <?php echo esc_attr($shortcode_element);?>" id="<?php echo cs_allow_special_char($name.$cs_counter);?>" data-shortcode-template="[<?php echo esc_attr( CS_SC_COUNTERS ) ;?> {{attributes}}]{{content}}[/<?php echo esc_attr( CS_SC_COUNTERS ) ;?>]" style="display: none;">
    <div class="cs-heading-area">
      <h5><?php _e('Edit Counter Options','luxury-hotel');?></h5>
    ...
    <div class="cs-pbwp-content">
      <div class="cs-wrapp-clone cs-shortcode-wrapp cs-pbwp-content">
           ...
           <li class="to-field">
            <div class="select-style">
              <select name="counter_style[]" class="dropdown" onchange="cs_counter_view_type(this.value,'<?php echo cs_allow_special_char($counter_count)?>')" >
                <option value="classic" <?php if($counter_style=="classic")echo "selected";?> ><?php _e('Classic View','luxury-hotel');?></option>
        ...
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Choose Icon/Image','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <div class="select-style">
			  ...
			  <option <?php if($counter_icon_type=="image")echo "selected";?> value="image" ><?php _e('Image','luxury-hotel');?></option>
                <option <?php if($counter_icon_type=="icon")echo "selected";?> value="icon" ><?php _e('Icon','luxury-hotel');?></option>
                
              </select>
			  
              <div class='left-info'><p><?php _e('Choose an icon/image for the counter','luxury-hotel');?></p></div>
            </div>
          </li>
        </ul>
        <ul class="form-elements" style="display:none">
          ...
          <ul class="form-elements">
            ...
            <li class="to-field select-style">
              <select class="counter_icon_size" name="counter_icon_size[]">
                ...
                <option value="icon-2x" <?php if($counter_icon_size == 'icon-2x'){echo 'selected="selected"';}?>><?php _e('Small','luxury-hotel');?></option>
                <option value="icon-3x" <?php if($counter_icon_size == 'icon-3x'){echo 'selected="selected"';}?>><?php _e('Medium','luxury-hotel');?></option>
                <option value="icon-4x" <?php if($counter_icon_size == 'icon-4x'){echo 'selected="selected"';}?>><?php _e('Large','luxury-hotel');?></option>
                <option value="icon-5x" <?php if($counter_icon_size == 'icon-5x'){echo 'selected="selected"';}?>><?php _e('Extra Large','luxury-hotel');?></option>
              </select>
        ...
        <div class="selected_image_type<?php echo esc_attr($counter_count)?> " id="selected_view_icon_image_type<?php echo esc_attr($counter_count)?>" <?php if($counter_style <> "icon-border" &&  $counter_icon_type == "image"){ echo 'style="display:block"';} else { echo 'style="display:none"';}?>>
          <ul class="form-elements">
            ...
            <li class="to-field">
              ...
              <input name="cs_counter_logo<?php echo esc_attr($random_id);?>"  type="button" class="uploadMedia left" value="<?php _e('Browse','luxury-hotel');?>"/>
            </li>
          </ul>
          <div class="page-wrap" style="overflow:hidden; display:<?php echo cs_allow_special_char($cs_counter_logo) && trim($cs_counter_logo) !='' ? 'inline' : 'none';?>"" id="cs_counter_logo<?php echo esc_attr($random_id);?>_box" >
        ...
        <ul class="form-elements bcevent_title">
          <li class="to-label">
            <label><?php _e('set number','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <div class="input-sec">
        ...
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Sub Title','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <input type="text"  name="counter_title[]" value="<?php echo cs_allow_special_char($counter_title);?>" class="txtfield"  />
            <div class='left-info'><p><?php _e('enter a sub title for the counter','luxury-hotel');?></p></div>
          </li>
        </MS
        ><ul class="form-elements">
          <li class="to-label">
        ...
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Border Frame','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <div class="select-style">
              <select name="counter_border[]" class="dropdown">
                <option <?php if($counter_border=="on")echo "selected";?> value="on" ><?php _e('Yes','luxury-hotel');?></option>
                <option <?php if($counter_border=="off")echo "selected";?> value="off" ><?php _e('No','luxury-hotel');?></option>
              </select>
             <div class='left-info'> <p><?php _e('set yes/no border frame form the dropdown','luxury-hotel');?> </p></div>
            </div>
          </li>
        </ul>
        </div>
