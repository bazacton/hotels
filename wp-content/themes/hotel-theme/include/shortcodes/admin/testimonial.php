...
if ( ! function_exists( 'cs_pb_testimonials' ) ) {
    function cs_pb_testimonials($die = 0){
        ...
        $defaults = array('column_size'=>'1/1','testimonial_text_color'=>'','testimonial_style'=>'','cs_testimonial_text_align'=>'','cs_testimonial_section_title'=>'',		'cs_testimonial_class'=>'');
...
<div id="<?php echo cs_allow_special_char($name.$cs_counter)?>_del" class="column  parentdelete <?php echo cs_allow_special_char($coloumn_class);?> <?php echo cs_allow_special_char($shortcode_view);?>" item="testimonials" data="<?php echo cs_element_size_data_array_index($testimonials_element_size)?>" >
  ...
  <div class="cs-wrapp-class-<?php echo cs_allow_special_char($cs_counter)?> <?php echo cs_allow_special_char($shortcode_element);?>" id="<?php echo cs_allow_special_char($name.$cs_counter)?>" style="display: none;">
    <div class="cs-heading-area">
      <h5><?php _e('Edit Testimonials Options','luxury-hotel');?></h5>
      ...
      <div class="cs-clone-append cs-pbwp-content">
		  <div class="cs-wrapp-tab-box">
				<div id="shortcode-item-<?php echo cs_allow_special_char($cs_counter);?>" data-shortcode-template="{{child_shortcode}} [/<?php echo esc_attr( CS_SC_TESTIMONIALS );?>]" data-shortcode-child-template="[<?php echo esc_attr( CS_SC_TESTIMONIALSITEM );?> {{attributes}}] {{content}} [/<?php echo esc_attr( CS_SC_TESTIMONIALSITEM );?>]">
						  ...
						  <li class='to-field'>
							<div class='input-sec select-style'>
							  <select name='testimonial_style[]' class='dropdown'>
								<option value='simple' <?php if($testimonial_style == 'simple'){echo 'selected';}?>><?php _e('list view','luxury-hotel');?></option>
						     	<option value='slider' <?php if($testimonial_style == 'slider'){echo 'selected';}?>><?php _e('Slider view','luxury-hotel');?></option>
                                <option value='modren-slider' <?php if($testimonial_style == 'modren-slider'){echo 'selected';}?>><?php _e('Modern Slider view','luxury-hotel');?></option>
							  </select>
							</div>
							<div class='left-info'>
							  <p><?php _e('Testimonial Style','luxury-hotel');?></p>
							</div>
						  </li>
						</ul>
						<ul class="form-elements" style="display:none">
						  ...
						  <li class="to-field select-style">
							<select name="cs_testimonial_text_align[]" class="dropdown" >
							  <option <?php if($cs_testimonial_text_align=="left")echo "selected";?> ><?php _e('left','luxury-hotel');?></option>
									</select>
							<p> <?php _e('This will not apply in Slider View','luxury-hotel');?></p>
						  </li>
						</ul>
						<ul class="form-elements">
						  <li class="to-label">
				  ...
				  <div class='cs-wrapp-clone cs-shortcode-wrapp cs-pbwp-content'
				   id="cs_infobox_<?php echo cs_allow_special_char($rand_string);?>">
					...
					<ul class='form-elements'>
					  <li class='to-label'>
						<label><?php _e('Company','luxury-hotel');?></label>
					  </li>
					  <li class='to-field'>
						<div class='input-sec'>
						...
						<div class='left-info'>
						  <p><?php _e('Company Name','luxury-hotel');?></p>
						</div>
					  </li>
					</ul>
					<ul class="form-elements">
					  ...
					  <li class="to-field">
						...
						<input name="testimonial_img<?php echo cs_allow_special_char($rand_string)?>"  type="button" class="uploadMedia left" value="<?php _e('Browse','luxury-hotel');?>"/>
					  </li>
					</ul>
				...
				<div class="wrapptabbox cs-pbwp-content" style="padding:0">
				  <div class="opt-conts">
					<ul class="form-elements">
					  <li class="to-field"> <a href="#" class="add_servicesss cs-main-btn" onclick="cs_shortcode_element_ajax_call('testimonials', 'shortcode-item-<?php echo cs_allow_special_char($cs_counter);?>', '<?php echo admin_url('admin-ajax.php');?>')"><i class="icon-plus-circle"></i><?php _e('Add testimonials','luxury-hotel');?></a> </li>
					  <div id="loading" class="shortcodeload"></div>
					</ul>
					<?php if(isset($_POST['shortcode_element']) && $_POST['shortcode_element'] == 'shortcode'){?>
					<ul class="form-elements insert-bg noborder">
					  <li class="to-field"> <a class="insert-btn cs-main-btn" onclick="javascript:Shortcode_tab_insert_editor('<?php echo str_replace('cs_pb_','',$name);?>','shortcode-item-<?php echo cs_allow_special_char($cs_counter);?>','<?php echo cs_allow_special_char($filter_element);?>')" ><?php _e('Insert','luxury-hotel');?></a> </li>
					</ul>
