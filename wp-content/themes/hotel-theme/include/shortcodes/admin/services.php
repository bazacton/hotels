...
<div id="<?php echo esc_attr($name.$cs_counter)?>_del" class="column  parentdelete <?php echo esc_attr($coloumn_class);?> <?php echo esc_attr($shortcode_view);?>" item="services" data="<?php echo cs_element_size_data_array_index($services_element_size)?>" >
  ...
  <div class="cs-wrapp-class-<?php echo esc_attr($cs_counter)?> <?php echo esc_attr($shortcode_element);?>" id="<?php echo esc_attr($name.$cs_counter)?>" data-shortcode-template="[<?php echo esc_attr( CS_SC_SERVICES ) ;?> {{attributes}}]{{content}}[/<?php echo esc_attr( CS_SC_SERVICES ) ;?>]" style="display: none;">
    <div class="cs-heading-area">
      <h5><?php _e('Edit Services Options','luxury-hotel');?></h5>
    ...
    <div class="cs-pbwp-content">
      <div class="cs-wrapp-clone cs-shortcode-wrapp cs-pbwp-content">
        <ul class='form-elements' style="display:none">
          <li class='to-label'>
            <label><?php _e('Choose View','luxury-hotel');?></label>
          </li>
          <li class='to-field select-style'>
            <div class='input-sec'>
              <select name='cs_service_type[]' class='dropdown' id="cs_service_type-<?php echo esc_attr($rand_counter)?>" onchange="cs_service_toggle_view(this.value,'<?php echo esc_attr($rand_counter);?>', jQuery(this))">
                <option value='modern' <?php if($cs_service_type == 'modern'){echo 'selected="selected"';}?>><?php _e('Modern','luxury-hotel');?></option>
                </select>
              <p class='left-info'><?php _e('Set a view from the dropdown','luxury-hotel');?></p>
            </div>
          </li>
        </ul>
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Border Right','luxury-hotel');?></label>
          </li>
          <li class="to-field select-style">
            <select name="cs_service_border_right[]">
              <option value="yes" <?php if($cs_service_border_right == 'yes'){echo 'selected="selected"';}?>><?php _e('Yes','luxury-hotel');?></option>
              <option value="no" <?php if($cs_service_border_right == 'no'){echo 'selected="selected"';}?>><?php _e('No','luxury-hotel');?></option>
            
            </select>
            <div class='left-info'><p><?php _e('This setting will apply only for "Classic Border" view','luxury-hotel');?></p></div>
          </li>
        </ul>
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Choose','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <div class="select-style">
              <select name="cs_service_icon_type[]" class="dropdown" onchange="cs_service_toggle_image(this.value,'<?php echo esc_attr($rand_counter);?>', jQuery(this))">
                <option <?php if($cs_service_icon_type=="icon")echo "selected";?> value="icon" ><?php _e('Icon','luxury-hotel');?></option>
                <option <?php if($cs_service_icon_type=="image")echo "selected";?> value="image" ><?php _e('Image','luxury-hotel');?></option>
              </select>
              <div class='left-info'><p><?php _e('Choose a icon/image type form the dropdown','luxury-hotel');?></p></div>
            </div>
          </li>
        </ul>
        <div class="selected_icon_type" id="selected_icon_type<?php echo esc_attr($rand_counter)?>" <?php if($cs_service_icon_type<>"image"){ echo 'style="display:block"';} else { echo 'style="display:none"';}?>>
          <ul class='form-elements' id="cs_infobox_<?php echo esc_attr($rand_counter);?>">
            <li class='to-label'>
              <label><?php _e('Choose Icon','luxury-hotel');?></label>
            </li>
            <li class="to-field">
              <?php cs_fontawsome_icons_box($cs_service_icon,$rand_counter,'cs_service_icon');?>
              <div class='left-info'><p><?php _e('Select the fontawsome Icons you would like to add to your menu items','luxury-hotel');?></p></div>
            </li>
          </ul>
          <ul class="form-elements">
            <li class="to-label">
              <label><?php _e('Icon Color','luxury-hotel');?></label>
            </li>
            <li class="to-field">
              <div class='input-sec'>
                <input type="text" name="cs_service_icon_color[]" class="bg_color"  value="<?php echo esc_attr($cs_service_icon_color);?>" />
                <div class='left-info'><p><?php _e('Set custom colour for icon','luxury-hotel');?></p></div>
              </div>
            </li>
          </ul>
        ...
        <div class="selected_image_type" id="selected_image_type<?php echo esc_attr($rand_counter);?>" <?php if($cs_service_icon_type=="image"){ echo 'style="display:block"';} else { echo 'style="display:none"';}?>>
          <ul class="form-elements">
            ...
            <li class="to-field">
              ...
              <input name="service_bg_image<?php echo esc_attr($rand_counter);?>"  type="button" class="uploadMedia left" value="<?php _e('Browse','luxury-hotel');?>"/>
            </li>
          </ul>
        ...
        <ul class="form-elements"  id="modern-size-<?php echo esc_attr($rand_counter);?>" style=" <?php echo esc_attr($cs_service_type) == '' || $cs_service_type == 'modern'? 'display:block;' : 'display:none;' ;?>">
          <li class="to-label">
            <label><?php _e('Icon Size','luxury-hotel');?></label>
          </li>
          <li class="to-field select-style">
            <select class="service_icon_size" name="service_icon_size[]">
              <option value="icon-2x" <?php if($service_icon_size == 'icon-2x'){echo 'selected="selected"';}?>><?php _e('Small','luxury-hotel');?></option>
              <option value="icon-3x" <?php if($service_icon_size == 'icon-3x'){echo 'selected="selected"';}?>><?php _e('Medium','luxury-hotel');?></option>
              <option value="icon-4x" <?php if($service_icon_size == 'icon-4x'){echo 'selected="selected"';}?>><?php _e('Large','luxury-hotel');?></option>
              <option value="icon-5x" <?php if($service_icon_size == 'icon-5x'){echo 'selected="selected"';}?>><?php _e('Extra Large','luxury-hotel');?></option>
            </select>
            <div class='left-info'><p><?php _e('Select Icon Size','luxury-hotel');?></p></div>
          </li>
        </ul>
        ...
        <ul class="form-elements" id="service-position-modern-<?php echo esc_attr($rand_counter);?>" style=" <?php echo trim($cs_service_type) == '' || $cs_service_type == 'modern'? 'display:block;' : 'display:none;' ;?>">
          ...
          <li class="to-field select-style">
            <select class="service_postion" name="cs_service_postion_modern[]">
              <option value="left" <?php if($cs_service_postion_modern == 'left'){echo 'selected="selected"';}?>><?php _e('left','luxury-hotel');?></option>
              <option value="center" <?php if($cs_service_postion_modern == 'center'){echo 'selected="selected"';}?>><?php _e('Center','luxury-hotel');?></option>
              <option value="right" <?php if($cs_service_postion_modern == 'right'){echo 'selected="selected"';}?>><?php _e('Right','luxury-hotel');?></option>
              <option value="top-left" <?php if($cs_service_postion_modern == 'top-left'){echo 'selected="selected"';}?>><?php _e('Top Left','luxury-hotel');?></option>
            </select>
            <div class='left-info'><p><?php _e('Give the position','luxury-hotel');?></p></div>
          </li>
        </ul>
        <ul class="form-elements" id="service-position-classic-<?php echo esc_attr($rand_counter);?>" style=" <?php echo trim($cs_service_type) == '' || $cs_service_type == 'modern'? 'display:none;' : 'display:block;' ;?>">
          ...
          <li class="to-field select-style">
            <select class="service_postion" name="cs_service_postion_classic[]">
              <option value="left" <?php if($cs_service_postion_classic == 'left'){echo 'selected="selected"';}?>><?php _e('Left','luxury-hotel');?></option>
              <option value="right" <?php if($cs_service_postion_classic == 'right'){echo 'selected="selected"';}?>><?php _e('Right','luxury-hotel');?></option>
