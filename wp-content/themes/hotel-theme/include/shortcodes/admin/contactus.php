...
<div id="<?php echo esc_attr($name.$cs_counter)?>_del" class="column  parentdelete <?php echo esc_attr($coloumn_class);?> <?php echo esc_attr($shortcode_view);?>" item="contactus" data="<?php echo cs_element_size_data_array_index($contactus_element_size)?>" >
  ...
  <div class="cs-wrapp-class-<?php echo intval($cs_counter)?> <?php echo esc_attr($shortcode_element);?>" id="<?php echo esc_attr($name.$cs_counter)?>" data-shortcode-template="[<?php echo esc_attr( CS_SC_CONTACTUS );?> {{attributes}}]" style="display: none;">
    <div class="cs-heading-area">
      <h5><?php _e('Edit Contact Form Options','luxury-hotel');?></h5>
        ...
        <ul class="form-elements">
          ...
          <li class="to-field select-style">
            <select class="cs_contactus_view" id="cs_contactus_view" name="cs_contactus_view[]">
              <option <?php if($cs_contactus_view == "plain")echo "selected";?> value="plain"><?php _e('Plain','luxury-hotel');?></option>
              <option <?php if($cs_contactus_view == "fancy")echo "selected";?> value="fancy"><?php _e('Fancy','luxury-hotel');?></option>
            </select>
          </li>
        </ul>
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Label On/Off','luxury-hotel');?></label>
          </li>
          <li class="to-field select-style">
            <select class="cs_contactus_label" id="cs_contactus_label" name="cs_contactus_label[]">
              <option <?php if($cs_contactus_label == "on")echo "selected";?> value="on"><?php _e('On','luxury-hotel');?></option>
              <option <?php if($cs_contactus_label == "off")echo "selected";?> value="off"><?php _e('Off','luxury-hotel');?></option>
            </select>
          </li>
        </ul>
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Send To','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <input type="text" name="cs_contactus_send[]" class="txtfield" value="<?php echo esc_attr($cs_contactus_send);?>" />
            <p><?php _e('add a email which you want to receive email','luxury-hotel');?></p>
          </li>
        </ul>
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Success Message','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <input type="text" name="cs_success[]" class="txtfield" value="<?php echo esc_attr($cs_success);?>" />
            <p><?php _e('set a message if your email sent successfully','luxury-hotel');?> </p>
          </li>
        </ul>
        <ul class="form-elements">
          <li class="to-label">
            <label><?php _e('Error Message','luxury-hotel');?></label>
          </li>
          <li class="to-field">
            <input type="text" name="cs_error[]" class="txtfield" value="<?php echo esc_attr($cs_error);?>" />
            <p><?php _e('set a message for error message','luxury-hotel');?></p>
          </li>
        </ul>
